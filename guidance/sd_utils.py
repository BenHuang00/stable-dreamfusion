...
class StableDiffusion(nn.Module):
    ...
    def train_step(self, text_embeddings, pred_rgb, guidance_scale=100, as_latent=False, grad_scale=1,
        ...
        if self.depthfm_model is not None:
            ...
            ratio = torch.cat([(self.depthfm_ratio * (1 - t / self.max_step))])
            print(f'[INFO] depthfm ratio: {ratio}')
            print(f'[INFO] depthfm ratio: {ratio.item()}')
            loss += ratio * depth_loss
            if wandb.run is not None:
    ...
    def get_depthfm_loss(self, pred_x0, depth, latents_noisy, num_steps: int = 4, save_guidance_path=None):
        ...
        with torch.no_grad():
            ...
            if self.depthfm_model.noising_step > 0:
